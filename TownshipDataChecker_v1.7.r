...
##### Version 1.8 updates ####
#Added code to handle Qualtrics output from GLOV1.0 dataset
...
#  lead to the "Township Data Checks" folder on your computer.
  ...
  setwd("C:/Users/paleolab/Dropbox/PalEON Lab Team") #middle desktop  
...
town <- read.csv("./Qualtrics Checks By Reader/Checked Townships/28N1W4_QualtricsCleaned_Checked.csv", header = TRUE, sep=",", na.strings = "NULL", stringsAsFactors = FALSE)
township.name <- "28N1W4_JoDaviess_IL"
...
####################################################################################################################
####################################################################################################################
### IF YOU ARE CHECKING TOWNSHIPS FROM GLOV1.0 FROM QUALTRICS GO DOWN TO LINE 820 AND RUN THE CODE FROM THERE ######
####################################################################################################################
####################################################################################################################
...
match.view <- town[which(match == "FALSE"),c("page","bearing", "degrees","bearingdir","chainstree","cornerid","entry_id")]
match.view <- match.view[order(match.view$page),]
acute.view <- town[which(acute == "FALSE"),c("page","bearing", "degrees","bearingdir","chainstree","cornerid","entry_id")]
acute.view <- acute.view[order(acute.view$page),]
match.view2 <- town[which(match2 == "FALSE"),c("page","bearing2", "degrees2","bearingdir2","chainstree2","cornerid","entry_id")]
match.view2 <- match.view2[order(match.view2$page),]
acute.view2 <- town[which(acute2 == "FALSE"),c("page","bearing2", "degrees2","bearingdir2","chainstree2","cornerid","entry_id")]
acute.view2 <- acute.view2[order(acute.view2$page),]
match.view3 <- town[which(match3 == "FALSE"),c("page","bearing3", "degrees3","bearingdir3","chainstree3","cornerid","entry_id")]
match.view3 <- match.view3[order(match.view3$page),]
acute.view3 <- town[which(acute3 == "FALSE"),c("page","bearing3", "degrees3","bearingdir3","chainstree3","cornerid","entry_id")]
acute.view3 <- acute.view3[order(acute.view3$page),]
match.view4 <- town[which(match4 == "FALSE"),c("page","bearing4", "degrees4","bearingdir4","chainstree4","cornerid","entry_id")]
match.view4 <- match.view4[order(match.view4$page),]
acute.view4 <- town[which(acute4 == "FALSE"),c("page","bearing4", "degrees4","bearingdir4","chainstree4","cornerid","entry_id")]
acute.view4 <- acute.view4[order(acute.view4$page),]
...
compass.view <- town[which(compass == "FALSE"),c("page","bearing", "degrees","bearingdir","chainstree","cornerid","entry_id")]
compass.view <- compass.view[order(compass.view$page),]
...
compassb <- town[which(compassa == "TRUE"),c("page","bearing","degrees", "bearingdir","chainstree","cornerid","entry_id")]
compassc <- compassb[which(compassb$chainstree >= 0),]
compassc <- compassc[order(compassc$page),]



## repeat for tree 2
compass2 <- !is.na(town$chainstree2) == (town$bearingdir2 == "E" | town$bearingdir2 == "W" | town$bearingdir2 == "99999" | town$bearingdir2 == "88888")
compass.view2 <- town[which(compass2 == "FALSE"),c("page","bearing2", "degrees2","bearingdir2","chainstree2","cornerid","entry_id")]
compass.view2 <- compass.view2[order(compass.view2$page),]
...
compass2b <- town[which(compass2a == "TRUE"),c("page","bearing2","degrees2", "bearingdir2","chainstree2","cornerid","entry_id")]
compass2c <- compass2b[which(compass2b$chainstree2 >= 0),]
compass2c <- compass2c[order(compass2c$page),]

## repeat for tree 3
compass3 <- !is.na(town$chainstree3) == (town$bearingdir3 == "E" | town$bearingdir3 == "W" | town$bearingdir3 == "99999" | town$bearingdir3 == "88888")
compass.view3 <- town[which(compass3 == "FALSE"),c("page","bearing3", "degrees3","bearingdir3","chainstree3","cornerid","entry_id")]
compass.view3 <- compass.view3[order(compass.view3$page),]
#compass.view3a <- town[is.na(compass3),c("bearing3", "degrees3","bearingdir3","chainstree3","cornerid","entry_id")]
compass3a <- is.na(compass3)
compass3b <- town[which(compass3a == "TRUE"),c("page","bearing3","degrees3", "bearingdir3","chainstree3","cornerid","entry_id")]
compass3c <- compass3b[which(compass3b$chainstree3 >= 0),]
compass3c <- compass3c[order(compass3c$page),]


## repeat for tree 4
compass4 <- !is.na(town$chainstree4) == (town$bearingdir4 == "E" | town$bearingdir4 == "W" | town$bearingdir4 == "99999" | town$bearingdir4 == "88888")
compass.view4 <- town[which(compass4 == "FALSE"),c("page","bearing4", "degrees4","bearingdir4","chainstree4","cornerid","entry_id")]
compass.view4 <- compass.view4[order(compass.view4$page),]
compass4a <- is.na(compass4)
compass4b <- town[which(compass4a == "TRUE"),c("page","bearing4","degrees4", "bearingdir4","chainstree4","cornerid","entry_id")]
compass4c <- compass4b[which(compass4b$chainstree4 >= 0),]
compass4c <- compass4c[order(compass4c$page),]
...
output <- list(metadata = metadata,
               ...
               bearingdir1NAs = if(nrow(compassc)>0) compassc else "none",
               bearingdir2 = if(nrow(compass.view2)>0) compass.view2 else "none",
               bearingdir2NAs = if(nrow(compass2c)>0) compass2c else "none",
               bearingdir3 = if(nrow(compass.view3)>0) compass.view3 else "none",
               bearingdir3NAs = if(nrow(compass3c)>0) compass3c else "none",
               bearingdir4 = if(nrow(compass.view4)>0) compass.view4 else "none",
               bearingdir4NAs = if(nrow(compass4c)>0) compass4c else "none",
               ...
               diameters = "check any diameters over 60 or less than 4 inches",
               degrees = "check any degrees 90 or greater",
               chainstrees = "check for outliers in chainstrees",
               ...
               general.notes = "scan the general notes for anything out of the ordinary",
...
town = town[order(town$verbatim, decreasing = TRUE,town$notree),] 
write.csv(town, paste0("./Qualtrics Checks By Reader/Checked Townships/QA_QC Checks/", township.name, "_Checked_RINPROGRESS.csv"), row.names = FALSE, na = "NA")


#############################################################################################################
#############################################################################################################
####  CODE TO CHECK DATA FROM GLOV1.0 QUALTRICS OUTPUT ######################################################
#############################################################################################################
#############################################################################################################

#after you have read in the town, township.name,and R.checker.version, then start with this code.

#GLOv1.0 files seem to have a number of extra rows with no data included. So you'll need to remove those rows.
#Open the town dataframe in R and see how many rows have actual data then use that info to create the correct dataframe below

town = town[-c(110:250),-c(80:163)] #first set of numbers are the rows to trim (e.g., 110:193) start with the 1st row with no data and go to the # of observations in the town dataframe
#second set of numbers (e.g., 80:163) are the columns to trim. There should be 79 columns that are kept.
...
#create a data frame with the sectiona/sectionb combinations that are incorrect
section.combo = data.frame(town2[wrong.section,]$sectiona,town2[wrong.section,]$sectionb,town2[wrong.section,]$cornerid,town2[wrong.section,]$entryid)
colnames(section.combo)<- c("sectiona","sectiona","cornerid","entry_id") 
section.combo
...
#create a data frame with the sectiona/sectionb combinations that are incorrect
section.comboA = data.frame(matched1b[wrong.sectionA,]$sectiona,matched1b[wrong.sectionA,]$sectionb,matched1b[wrong.sectionA,]$cornerid,matched1b[wrong.sectionA,]$entryid)
colnames(section.comboA)<- c("sectiona","sectionb","cornerid","entry_id") 
section.comboA
...
#create a data frame with the sectiona/sectionb combinations that are incorrect
section.comboB = data.frame(matched2b[wrong.sectionB,]$sectiona,matched2b[wrong.sectionB,]$sectionb,matched2b[wrong.sectionB,]$cornerid,matched2b[wrong.sectionB,]$entryid)
colnames(section.comboB)<- c("sectiona","sectionb","cornerid","entry_id") 
section.comboB
...
#create a data frame with the sectiona/sectionb combinations that are incorrect
qsection.comboA = data.frame(matched3b[wrong.qsectionA,]$sectiona,matched3b[wrong.qsectionA,]$sectionb,matched3b[wrong.qsectionA,]$cornerid,matched3b[wrong.qsectionA,]$entryid)
colnames(qsection.comboA)<- c("sectiona","sectionb","cornerid","entry_id") 
qsection.comboA
...
#create a data frame with the sectiona/sectionb combinations that are incorrect
qsection.comboB = data.frame(matched4b[wrong.qsectionB,]$sectiona,matched4b[wrong.qsectionB,]$sectionb,matched4b[wrong.qsectionB,]$cornerid,matched4b[wrong.qsectionB,]$entryid)
colnames(qsection.comboB)<- c("sectiona","sectionb","cornerid","entry_id") 
qsection.comboB
...
test = !(is.na(town$interiorsection)) & ( !(is.na(town$sectiona)) | !(is.na(town$sectionb)))
section.dup <- town[which(test == "TRUE"),c("sectiona","sectionb","interiorsection","interiordir","cornerid","entryid")]
section.dup
...
# filter out NA's in test (which represent non-border/interiorsections) and make report of invalid interiorsections
test3 <- test2 == FALSE & !is.na(town$interiorsection)
valid.interiorsection <- town[which(test3 == "TRUE"),c("interiorsection","interiordir","cornerid","entryid")]
valid.interiorsection
...
diameter.check = town[which(town$diameter >= 60 & town$diameter < 88888),c("cornerid","diameter","generalnotes","entryid")]
diameter.check2 = town[which(town$diameter2 >= 60 & town$diameter2 < 88888),c("cornerid","diameter2","generalnotes","entryid")]
diameter.check3 = town[which(town$diameter3 >= 60 & town$diameter3 < 88888),c("cornerid","diameter3","generalnotes","entryid")]
diameter.check4 = town[which(town$diameter4 >= 60 & town$diameter4 < 88888),c("cornerid","diameter4","generalnotes","entryid")]
...
#Get all the entries with bearings that are not N,S,88888,99999 and degrees that are >90
match.view <- town[which(match == "FALSE"),c("page","bearing", "degrees","bearingdir","chainstree","cornerid","entryid")]
match.view <- match.view[order(match.view$page),]
acute.view <- town[which(acute == "FALSE"),c("page","bearing", "degrees","bearingdir","chainstree","cornerid","entryid")]
acute.view <- acute.view[order(acute.view$page),]
match.view2 <- town[which(match2 == "FALSE"),c("page","bearing2", "degrees2","bearingdir2","chainstree2","cornerid","entryid")]
match.view2 <- match.view2[order(match.view2$page),]
acute.view2 <- town[which(acute2 == "FALSE"),c("page","bearing2", "degrees2","bearingdir2","chainstree2","cornerid","entryid")]
acute.view2 <- acute.view2[order(acute.view2$page),]
match.view3 <- town[which(match3 == "FALSE"),c("page","bearing3", "degrees3","bearingdir3","chainstree3","cornerid","entryid")]
match.view3 <- match.view3[order(match.view3$page),]
acute.view3 <- town[which(acute3 == "FALSE"),c("page","bearing3", "degrees3","bearingdir3","chainstree3","cornerid","entryid")]
acute.view3 <- acute.view3[order(acute.view3$page),]
match.view4 <- town[which(match4 == "FALSE"),c("page","bearing4", "degrees4","bearingdir4","chainstree4","cornerid","entryid")]
match.view4 <- match.view4[order(match.view4$page),]
acute.view4 <- town[which(acute4 == "FALSE"),c("page","bearing4", "degrees4","bearingdir4","chainstree4","cornerid","entryid")]
acute.view4 <- acute.view4[order(acute.view4$page),]
...
# 2 - if bearingdir has value (True = 1 of 4 possible values (N, S, 99999, 88888); False = no data/blank or invalid value)
compass <- !is.na(town$chainstree) == (town$bearingdir == "E" | town$bearingdir == "W" | town$bearingdir == "99999" | town$bearingdir == "88888")
compass.view <- town[which(compass == "FALSE"),c("page","bearing", "degrees","bearingdir","chainstree","cornerid","entryid")]
compass.view <- compass.view[order(compass.view$page),]
#compassa <- is.na(town$bearingdir) == (town$bearingdir == "E" | town$bearingdir == "W" | town$bearingdir == "99999" | town$bearingdir == "88888" |town$chainstree == "NA")
#compass.viewa <- town[is.na(compassa),c("bearing", "degrees","bearingdir","chainstree","cornerid","entryid")]

compassa <- is.na(compass)
compassb <- town[which(compassa == "TRUE"),c("page","bearing","degrees", "bearingdir","chainstree","cornerid","entryid")]
compassc <- compassb[which(compassb$chainstree >= 0),]
compassc <- compassc[order(compassc$page),]
...
## repeat for tree 2
compass2 <- !is.na(town$chainstree2) == (town$bearingdir2 == "E" | town$bearingdir2 == "W" | town$bearingdir2 == "99999" | town$bearingdir2 == "88888")
compass.view2 <- town[which(compass2 == "FALSE"),c("page","bearing2", "degrees2","bearingdir2","chainstree2","cornerid","entryid")]
compass.view2 <- compass.view2[order(compass.view2$page),]
#compass2a <- is.na(town$bearingdir2) == (town$bearingdir2 == "E" | town$bearingdir2 == "W" | town$bearingdir2 == "99999" | town$bearingdir2 == "88888" | !is.na(town$chainstree2))
#compass.view2a <- town[is.na(compass2),c("bearing2", "degrees2","bearingdir2","chainstree2","cornerid","entryid")]
compass2a <- is.na(compass2)
compass2b <- town[which(compass2a == "TRUE"),c("page","bearing2","degrees2", "bearingdir2","chainstree2","cornerid","entryid")]
compass2c <- compass2b[which(compass2b$chainstree2 >= 0),]
compass2c <- compass2c[order(compass2c$page),]

## repeat for tree 3
compass3 <- !is.na(town$chainstree3) == (town$bearingdir3 == "E" | town$bearingdir3 == "W" | town$bearingdir3 == "99999" | town$bearingdir3 == "88888")
compass.view3 <- town[which(compass3 == "FALSE"),c("page","bearing3", "degrees3","bearingdir3","chainstree3","cornerid","entryid")]
compass.view3 <- compass.view3[order(compass.view3$page),]
#compass.view3a <- town[is.na(compass3),c("bearing3", "degrees3","bearingdir3","chainstree3","cornerid","entryid")]
compass3a <- is.na(compass3)
compass3b <- town[which(compass3a == "TRUE"),c("page","bearing3","degrees3", "bearingdir3","chainstree3","cornerid","entryid")]
compass3c <- compass3b[which(compass3b$chainstree3 >= 0),]
compass3c <- compass3c[order(compass3c$page),]
...
## repeat for tree 4
compass4 <- !is.na(town$chainstree4) == (town$bearingdir4 == "E" | town$bearingdir4 == "W" | town$bearingdir4 == "99999" | town$bearingdir4 == "88888")
compass.view4 <- town[which(compass4 == "FALSE"),c("page","bearing4", "degrees4","bearingdir4","chainstree4","cornerid","entryid")]
compass.view4 <- compass.view4[order(compass.view4$page),]
compass4a <- is.na(compass4)
compass4b <- town[which(compass4a == "TRUE"),c("page","bearing4","degrees4", "bearingdir4","chainstree4","cornerid","entryid")]
compass4c <- compass4b[which(compass4b$chainstree4 >= 0),]
compass4c <- compass4c[order(compass4c$page),]
...
#Get all the entries with chainstrees that do not have decimals of 0.00, 0.25, 0.50, 0.75 or NA
#In the future, may want to add options for 0.33 or 0.66/0.67 for 1/3 or 2/3 fractions in the links
decimalstree1 <-newchain[which(newchain$floor.town.chainstree. != 0.50 & newchain$floor.town.chainstree. !=0.25 & newchain$floor.town.chainstree. !=0.00 & newchain$floor.town.chainstree. !=0.75 & !is.na(newchain$floor.town.chainstree.)),c("chainstree","cornerid","entryid")]
decimalstree2 <-newchain2[which(newchain2$floor.town.chainstree2. != 0.50 & newchain2$floor.town.chainstree2. !=0.25 & newchain2$floor.town.chainstree2. !=0.00 & newchain2$floor.town.chainstree2. !=0.75 & !is.na(newchain2$floor.town.chainstree2.)),c("chainstree2","cornerid","entryid")]
decimalstree3 <-newchain3[which(newchain3$floor.town.chainstree3. != 0.50 & newchain3$floor.town.chainstree3. !=0.25 & newchain3$floor.town.chainstree3. !=0.00 & newchain3$floor.town.chainstree3. !=0.75 & !is.na(newchain3$floor.town.chainstree3.)),c("chainstree3","cornerid","entryid")]
decimalstree4 <-newchain4[which(newchain4$floor.town.chainstree4. != 0.50 & newchain4$floor.town.chainstree4. !=0.25 & newchain4$floor.town.chainstree4. !=0.00 & newchain4$floor.town.chainstree4. !=0.75 & !is.na(newchain4$floor.town.chainstree4.)),c("chainstree4","cornerid","entryid")]
...
# Check for invalid values in "refcorner" or NAs
refcornertest = town[!(town$refcorner %in% reference.corners),c("refcorner","cornerid","entryid")]
refcornertest
...
#create a data frame with the cornerid/refcorner combinations that are incorrect
section.combo.ref = data.frame(refcorner.matchb[wrong.section.refcorner,]$refcorner,refcorner.matchb[wrong.section.refcorner,]$cornerid,refcorner.matchb[wrong.section.refcorner,]$sectiona,refcorner.matchb[wrong.section.refcorner,]$sectionb,refcorner.matchb[wrong.section.refcorner,]$interiorsection,refcorner.matchb[wrong.section.refcorner,]$interiordir,refcorner.matchb[wrong.section.refcorner,]$entryid)
colnames(section.combo.ref)<- c("refcorner","cornerid","sectiona","sectiona","interiorsection", "interiordir","entryid") 
section.combo.ref
...
#create a data frame with the cornerid/refcorner combinations that are incorrect
qsection.combo.ref = data.frame(qrefcorner.matchb[q_wrong.section.refcorner,]$refcorner,qrefcorner.matchb[q_wrong.section.refcorner,]$cornerid,qrefcorner.matchb[q_wrong.section.refcorner,]$sectiona,qrefcorner.matchb[q_wrong.section.refcorner,]$sectionb,qrefcorner.matchb[q_wrong.section.refcorner,]$interiorsection,qrefcorner.matchb[q_wrong.section.refcorner,]$interiordir,qrefcorner.matchb[q_wrong.section.refcorner,]$entryid)
colnames(qsection.combo.ref)<- c("refcorner","cornerid","sectiona","sectiona","interiorsection", "interiordir","entryid") 
qsection.combo.ref
...
#then checks the dimensions of the first new dataframe - if it is too small (i.e., there are no errors), the output lists the 
#second NA dataframe. If if the first new dataframe has errors then that gets listed in the output
new_sectionmerge2=cbind(new_sectionmerge$cornerid,"Section","mismatched cornerid and section",new_sectionmerge$entryid)
new_sectionmergeNA=cbind("NA", "Section", "mismatched cornerid and Section","NA")
if(dim(new_sectionmerge2)[2]<4){new_sectionmerge2=new_sectionmergeNA} else{new_sectionmerge2=new_sectionmerge2}
colnames(new_sectionmerge2)=c("cornerid","tyecorner","error","entryid")
new_sectionmerge2
...
#then checks the dimensions of the first new dataframe - if it is too small (i.e., there are no errors), the output lists the 
#second NA dataframe. If if the first new dataframe has errors then that gets listed in the output
new_quartermerge2=cbind(new_quartermerge$cornerid,"(1/4) Section", "mismatched cornerid and (1/4) section",new_quartermerge$entryid)
new_quartermergeNA=cbind("NA", "(1/4) Section", "mismatched cornerid and (1/4) Section","NA")
if(dim(new_quartermerge2)[2]<4){new_quartermerge2=new_quartermergeNA} else{new_quartermerge2=new_quartermerge2}
colnames(new_quartermerge2)=c("cornerid","typecorner","error","entryid")
new_quartermerge2
...
#then checks the dimensions of the first new dataframe - if it is too small (i.e., there are no errors), the output lists the 
#second NA dataframe. If if the first new dataframe has errors then that gets listed in the output
new_townshipmerge2=cbind(new_townshipmerge$cornerid,"Township","mismatched cornerid and Township",new_townshipmerge$entryid)
new_townshipmergeNA=cbind("NA", "Township","mismatched cornerid and Township","NA")
if(dim(new_townshipmerge2)[2]<4){new_townshipmerge2=new_townshipmergeNA} else{new_townshipmerge2=new_townshipmerge2}
colnames(new_townshipmerge2)=c("cornerid","typecorner","error","entry_id")
new_townshipmerge2
...
cornerid.dups = town[duplicated(town$cornerid)|duplicated(town$cornerid, fromLast = TRUE),c("sectiona","sectionb","interiorsection","interiordir","refcorner","traveldir","secondpass","chains","typecorner","cornerid","entryid")]
cornerid.dups = cornerid.dups[order(cornerid.dups$cornerid),] 
cornerid.dups
...
# if flagged for recheck, list error and fixed message, if present
recheck = town[which(town$recheck == "Yes"),c("cornerid","recheck","reason","entryid")]
recheck
recheck2 = town[which(town$recheck == "Y"),c("cornerid","recheck","reason","entryid")]
recheck2
...
output <- list(metadata = metadata,
               ...
               general.notes = "scan the general notes for anything out of the ordinary",
               illegible.missing = "search for entries with 88888s or 99999s. make sure there are notes in the General Notes column explaining these entries - Done?",
               reminder = "concatenate First and Last Names and label as 'reader_initials'"
)
...
#add species codes for trees 1-4
spcode = read.csv(file = "./Qualtrics Checks By Reader/SpeciesCode_Conversion.csv", header = TRUE, stringsAsFactors = FALSE)
for(i in 1:length(town$speciescode)){
  town$speciescode[i] = spcode[match(town$species[i], spcode$Species, nomatch = NA),2]
}

for(i in 1:length(town$speciescode2)){
  town$speciescode2[i] = spcode[match(town$species2[i], spcode$Species, nomatch = NA),2]
}

for(i in 1:length(town$speciescode3)){
  town$speciescode3[i] = spcode[match(town$species3[i], spcode$Species, nomatch = NA),2]
}

for(i in 1:length(town$speciescode4)){
  town$speciescode4[i] = spcode[match(town$species4[i], spcode$Species, nomatch = NA),2]
}


#change the "No tree" entries so "Yes" entries (which mean Yes there were trees) now is entered as "" blank and 
# "No" (which mean No trees) now is entered as "yes" (which mean yes there are no trees to match the format of the GLOv2.0 data)

for(i in 1:length(town$notree)){
  if(town$notree[i] == 'Yes'){ 
    town$notree[i] <- ''
  }
  else if(town$notree[i] == 'No'){
    town$notree[i] <- 'yes'
  }
}


#change colname "water-no tree" to "water" and change "No tree" entries to "" blank
colnames(town)[69] <- "water"

for(i in 1:length(town$water)){
  if(town$water[i] == 'No tree'){ 
    town$water[i] <- ''
  }
}  
