...
#  completed townships by exporting that township from Qualtrics (previously MySQL) to a csv.  The data checker will
...
town <- read.csv("./5S7E3_Hamilton_Checked.csv", header = TRUE, sep=",", na.strings = "NULL", stringsAsFactors = FALSE)
#need to re-save the csv because for some reason when the csv is first brought in, the NAs are not recognized
write.csv(town, "./5S7E3_Hamilton_Checked.csv", row.names = FALSE)
town <- read.csv("./5S7E3_Hamilton_Checked.csv", header = TRUE, sep=",", na.strings = "NULL", stringsAsFactors = FALSE)
township.name <- "5S7E3_Hamilton"

#example of changing int to num: town$volume <- as.numeric(as.character(town$volume))

town <- read.csv("./HamiltonTest.csv", header = TRUE, sep=",", na.strings = "NULL", stringsAsFactors = FALSE)
write.csv(town, "./HamiltonTest2.csv", row.names = FALSE)
town <- read.csv("./HamiltonTest2.csv", header = TRUE, sep=",", na.strings = "NULL", stringsAsFactors = FALSE)
...
combo <- read.csv("./sectionCombos.csv", stringsAsFactors = FALSE)

#in town remove the rows that have NAs in sectiona (column 19) and sectionb (column 20) 
#(these removed rows have data for the S and E borders which will are checked below in section FILL IN)
town2 = town[complete.cases(town[,19:20]),]
...
matched1b = matched1[complete.cases(matched1[ ,24]),]
...
colnames(section.comboA)<- c("sectiona","sectiona","cornerid","entry_id") 
section.comboA
...
matched2b = matched2[complete.cases(matched2[ ,25]),]
...
colnames(section.comboB)<- c("sectiona","sectiona","cornerid","entry_id") 
section.comboB
...
colnames(qsection.comboA)<- c("sectiona","sectiona","cornerid","entry_id") 
qsection.comboA
...
matched4b = matched4[complete.cases(matched4[ ,23]),]
...
colnames(qsection.comboB)<- c("sectiona","sectiona","cornerid","entry_id") 
qsection.comboB
